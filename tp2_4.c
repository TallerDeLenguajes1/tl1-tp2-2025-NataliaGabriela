...
void listarPCs(compu *p);
void mostrarMasVieja(compu *p);
void mostrarMasVeloz(compu *p);
int main()
{
    srand(time(NULL));
    ...
    *p_pc = cargarpuntero(p_pc, tipo);
    ...
    // fflush(stdin);
    // PC mas vieja
    mostrarMasVieja(p_pc);
    printf("\n---------------------------\n");
    // PC mas rapida
    mostrarMasVeloz(p_pc);
    scanf(" %c");
    return 0;
}
compu cargarpc(compu pc, char *tipo)
